...
//    val wire2 = "U62,R66,U55,R34,D71,R55,D58,R83"
    ...
    val intersections: ArrayList<Pair<Int,Int>> = ArrayList()
    ...
    wire1.split(",").stream()
        .forEach {d ->
            val direction = d[0]
            val length: Int = d.substring(1, d.length).toInt()
    ...
    wire2.split(",").stream()
        .forEach {d ->
            ...
            for (i in 1..length) {
                ...
                if (grid[xpos][ypos] == 1.toByte()) {
                    intersections.add(Pair(xpos, ypos))
                    grid[xpos][ypos] = -1
                } else {
                    grid[xpos][ypos] = 2
                }
    ...
    var shortestDistance: Int = 1_000_000
    intersections.forEach { p ->
        val xDistance = abs(startX - p.first)
        val yDistance = abs(startY - p.second)
    ...
    println("Shortest distance of intersection is $shortestDistance")

    val intersectionMap = hashMapOf<Pair<Int,Int>, Int>()
    intersections.forEach { p ->
        intersectionMap[p] = 0
    }
    ...
    wire1.split(",").stream()
        .forEach {d ->
            ...
            for (i in 1..length) {
                ...
                if (grid[xpos][ypos] == (-1).toByte()) {
                    intersectionMap[Pair(xpos,ypos)] = steps1
                }
            }
        }
    ...
    wire2.split(",").stream()
        .forEach {d ->
            ...
            for (i in 1..length) {
                ...
                if (grid[xpos][ypos] == (-1).toByte()) {
                    intersectionMap[Pair(xpos,ypos)] = intersectionMap[Pair(xpos,ypos)]!!.plus(steps2)
                }
            }
        }

    val minSteps = intersectionMap.values.min()
    println("Min steps is: $minSteps")
